...
narrowStepBasic :: (Unify t, Ord v, Observable v, MonadPlus m, MonadVariant m, MonadEnv m, Var m ~ v, t ~ TermF m) =>
...
narrow1P :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadPlus m) => [Rule t v] -> Term t v -> m ((Term t v, Position), Substitution t v)
narrow1P rr t= second (restrictTo (vars t)) `liftM` narrow1P' rr t

-- | narrowing to rigid normal form
#ifdef LOGICT
narrow :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadLogic m, Eq (Term t v)
          ) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
#else
narrow :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadPlus m, Eq (Term t v)) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
#endif
narrow  rr t = second (restrictTo (vars t)) `liftM` narrow' rr t

-- | narrowing transitive closure
narrows :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadPlus m) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
...
deriving instance (Functor t, Foldable t, Ord v, Observable v, Monad m) => MonadEnv (NarrowingM t v m)
...
run   :: (Enum v, Ord v, Observable v, Functor t, Foldable t, Monad m
...
-- | narrowing to rigid normal form
#ifdef LOGICT
narrow' :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadLogic m, Eq (Term t v)) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
...
qNarrow1P :: ( Ord v, Observable v, Enum v, Rename v, Unify t, MonadPlus m, Observable1 t
             ) => (Term t v -> Bool) -> [Rule t v] -> Term t v -> m ((Term t v, Position), Substitution t v)
...
qNarrowStepBasic :: (Unify t, Enum v, Ord v, Observable v, Observable1 t, MonadPlus m, MonadVariant m, MonadEnv m, Var m ~ v, t ~ TermF m) =>
                   (Term t v -> Bool) -> [Rule t v] -> Term t v -> m (Term t v, Position)
qNarrowStepBasic isQNF rr t = go (t, mempty, [])
...
#ifdef LOGICT
-- | Innermost narrowing
innNarrowing :: (Unify t, Ord v, Observable v, Enum v, Rename v, Var m ~ v, MonadLogic m) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
innNarrowing rr t = do
  (t', s) <- run (fixMP (innStepBasic rr >=> zonkM return)) t
  return (t', zonkSubst s)

-- | Innermost Basic Narrowing
innBnarrowing :: (Unify t, Ord v, Observable v, Enum v, Rename v, MonadLogic m) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
...
innStepBasic :: (Ord v, Observable v, Unify t, TermF m ~ t, Var m ~ v, MonadEnv m, MonadVariant m, MonadLogic m) => [Rule t v] -> Term t v -> m(Term t v)
...
narrowBounded :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadPlus m) => (Term t v -> Bool) -> [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
...
-- ** Basic Narrowing
narrow1Basic :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadPlus m) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
narrow1Basic = narrow1

narrowsBasic :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadPlus m) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
narrowsBasic rr t = second (restrictTo (vars t)) `liftM`
                    run (closureMP (liftM fst . narrowStepBasic rr) >=> zonkM return) t
#ifdef LOGICT
narrowBasic :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadLogic m) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
narrowBasic rr t = second (restrictTo (vars t)) `liftM`
                   run (fixMP (liftM fst . narrowStepBasic rr) >=> zonkM return) t
#else
narrowBasic :: (Ord v, Observable v, Enum v, Rename v, Unify t, Eq (Term t v), MonadPlus m) => [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
narrowBasic rr t = second (restrictTo (vars t)) `liftM`
                   run (fixM_Eq (liftM fst . narrowStepBasic rr) >=> zonkM return) t
#endif
narrowBasicBounded :: (Ord v, Observable v, Enum v, Rename v, Unify t, MonadPlus m) => (Term t v -> Bool) -> [Rule t v] -> Term t v -> m (Term t v, Substitution t v)
